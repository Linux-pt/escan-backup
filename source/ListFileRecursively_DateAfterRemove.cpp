...
/*this function list all files for date after remove*/
/*DateAfterRemove*/
extern sqlite3 *db;
extern int flag;
void ListFileRecursively_DateAfterRemove(char *basePath,int val)
{
	...
	//WriteLogForlibmwshare(2,"ListFileRecursively_DateAfterRemove started" );
	char path[1024]="";
	struct dirent *dp;
	...
	while ((dp = readdir(dir)) != NULL && !flag)
	{
		if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0)
		{
			...
			char date[1024]="";
			strcpy(date,ptr);
			DateAfterRemove(date,f_name,val);
			...
			ListFileRecursively_DateAfterRemove(path,val);
		}
	}

	//WriteLogForlibmwshare(2,"ListFileRecursively_DateAfterRemove ended" );
	closedir(dir);
}
/*this function will remove files after date*/
void DateAfterRemove(char *tmpdata_date,char *unzip,int val)
{
	//WriteLogForlibmwshare(2,"DateAfterRemove started" );
	signal(SIGINT, HandleSigint);
	...
	*date1 = *localtime(&date_seconds);
	...
	//WriteLogForlibmwshare(2,"DateAfterRemove ended" );
